...
/*
 * myledip.c — VD100 PL LED + PL Reset IRQ + PS_KEY Driver
 *
 * Manages PL_LED1 via AXI-lite MyLEDIP peripheral (0xA4000000).
 * Detects PL fabric reset via pl_ps_irq[0] (GIC SPI 84, level-high).
 * Handles PS_KEY GPIO interrupt for interactive LED toggle.
 * Exposes /dev/myledip character device and sysfs pl_led_state attribute.
 *
 * Hardware mapping (VD100 XCVE2302):
 *   PL_LED1  — E20, LVCMOS15, active-low  — AXI MyLEDIP offset 0x00
...
#define MYLEDIP_CTRL_REG        0x00    /* slv_reg0 — LED control, bit 0 */
#define MYLEDIP_STATUS_REG      0x04    /* slv_reg1 — status readback    */
...
/*
 * myledip_hw_set — write LED state to hardware and update shadow.
 * Must be called with mdev->lock held when invoked from IRQ context.
 */
static void myledip_hw_set(struct myledip_dev *mdev, u32 val)
{
	mdev->led_shadow = val ? 1 : 0;
	iowrite32(mdev->led_shadow, mdev->base + MYLEDIP_CTRL_REG);
}

/*
 * myledip_hw_get — read LED state directly from hardware register.
 * Used only during probe to reconstruct shadow after PS reboot.
 */
static u32 myledip_hw_get(struct myledip_dev *mdev)
{
	return ioread32(mdev->base + MYLEDIP_CTRL_REG) & 0x1;
}
...
{
	struct myledip_dev *mdev = dev_get_drvdata(dev);
...
{
	struct myledip_dev *mdev =
		container_of(work, struct myledip_dev, pl_restore.work);
	unsigned long flags;
...
/* ── Character device — /dev/myledip ─────────────────────────────────────── */

static int myledip_open(struct inode *inode, struct file *filp)
{
	filp->private_data = container_of(inode->i_cdev,
					   struct myledip_dev, cdev);
	return 0;
}
 ...
 */
static ssize_t myledip_read(struct file *filp, char __user *ubuf,
			     size_t count, loff_t *ppos)
{
	struct myledip_dev *mdev = filp->private_data;
	char kbuf[4];
	int len;
 ...
 * Existing userspace (echo 1 | tee /dev/myledip) continues to work.
 */
static ssize_t myledip_write(struct file *filp, const char __user *ubuf,
			      size_t count, loff_t *ppos)
{
	struct myledip_dev *mdev = filp->private_data;
	unsigned long flags;
...
static int myledip_probe(struct platform_device *pdev)
{
	 ...
	 */
	mdev->led_shadow = myledip_hw_get(mdev);
	...
	ret = devm_request_irq(&pdev->dev, irq, pl_reset_irq_handler,
			       IRQF_TRIGGER_RISING, "myledip_pl_reset", mdev);
	if (ret) {
	...
	cdev_init(&mdev->cdev, &myledip_fops);
	...
	/* Creates /dev/myledip — udev rule applies GROUP=pldev MODE=0660 */
	...
	if (ret) {
		...
		/* non-fatal — /dev/myledip still works */
	}

	dev_info(&pdev->dev,
		 "myledip probe complete — /dev/myledip ready, pl_ps_irq=%d\n",
		 mdev->irq_pl);
	return 0;
...
{
	struct myledip_dev *mdev = platform_get_drvdata(pdev);
	...
	dev_info(&pdev->dev, "myledip removed\n");
}
